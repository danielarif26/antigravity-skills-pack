...
Scrub list before merging anything from `the research-toolkit fork`: hardcoded `~/...` and `~/Documents/SecondBrain` paths, `langlive-line-oa` defaults, Discord `#langlive-line-oa`, zh-TW header defaults. From `the pillar-vault fork`: `~/...` paths, the eight-pillar layout, the `the pillar-vault fork` skill rename. From `the calendar/workflow fork`: Spanish section headers, "Interface School", the faith-oriented `area` taxonomy.
